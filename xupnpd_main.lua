...
function load_plugins(path,what)
    ...
    if d then
	    ...
	    -- añadida �linea para detectar que se cargaba
	    if cfg.debug>0 then print('Cargando:'..j..' Path:'..path) end
...
--+ Cambiada linea para concatenar / para que a la hora de cargar los profiles no fallará
if cfg.profiles then load_plugins(cfg.profiles..'/','profile') end
-- Original: if cfg.profiles then load_plugins(cfg.profiles..'/','profile') end
